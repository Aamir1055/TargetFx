...
const PositionsPage = () => {
  ...
  const { filterByActiveGroup, activeGroupFilters, getActiveGroupFilter, getGroupLogins, groups } = useGroups()
  ...
  const [columnOrder, setColumnOrder] = useState(() => {
    try {
      const saved = localStorage.getItem('positionsPageColumnOrder')
      if (saved) {
  ...
  useEffect(() => {
    try {
      if (columnOrder) localStorage.setItem('positionsPageColumnOrder', JSON.stringify(columnOrder))
    } catch {}
  }, [columnOrder])
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (activeGroupName) {
          const groupLogins = getGroupLogins(activeGroupName).map(l => Number(l)).filter(n => !Number.isNaN(n))
          netApiFilters.push({ field: 'login', operator: 'in', value: groupLogins.length > 0 ? groupLogins : [-1] })
        }
  ...
  }, [isAuthenticated, isMobile, showNetPositions, netCurrentPage, netItemsPerPage, netSortColumn, netSortDirection, netActiveSearch, groupByBaseSymbol, displayMode, activeGroupFilters, getActiveGroupFilter, getGroupLogins, groups, selectedLogin])
            ...
            <div className="bg-white rounded-xl shadow-sm border border-[#F2F2F7] p-2 hover:md:shadow-md transition-shadow">
              ...
              {isInitialPositionsLoading ? (
                ...
                <div className={`text-sm md:text-base font-bold flex items-center gap-1.5 leading-none ${
                  ...
                  <span className="inline-flex items-center leading-none flex-shrink-0" style={{ fontSize: '0.85em' }} aria-label={summaryStats.floatingINR >= 0 ? 'up' : 'down'}>{summaryStats.floatingINR >= 0 ? '▲' : '▼'}</span>
                  <span title={numericMode === 'compact' ? fmtMoneyFull(summaryStats.floatingINR) : undefined}>{fmtMoney(summaryStats.floatingINR)}</span>
                </div>
              )}
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-[#F2F2F7] p-2 hover:md:shadow-md transition-shadow">
              ...
              {isInitialPositionsLoading ? (
                ...
                <div className={`text-sm md:text-base font-bold flex items-center gap-1.5 leading-none ${
                  ...
                  <span className="inline-flex items-center leading-none flex-shrink-0" style={{ fontSize: '0.85em' }} aria-label={summaryStats.floatingUSD >= 0 ? 'up' : 'down'}>{summaryStats.floatingUSD >= 0 ? '▲' : '▼'}</span>
                  <span title={numericMode === 'compact' ? fmtMoneyFull(summaryStats.floatingUSD) : undefined}>{fmtMoney(summaryStats.floatingUSD)}</span>
                </div>
              )}
            </div>
          </div>
                      ...
                      <thead className="bg-blue-600 sticky top-0 shadow-md" style={{ zIndex: 30, backgroundColor: '#2563eb' }}>
                        <tr>
                          {clientNetVisibleColumns.login && (
                            <th 
                              className="px-3 py-3 text-left text-xs font-bold bg-blue-600 text-white uppercase tracking-wider cursor-pointer hover:bg-blue-700 transition-all select-none group sticky left-0"
                          ...
                          {clientNetVisibleColumns.symbol && (
                            <th 
                              className="px-3 py-3 text-left text-xs font-bold bg-blue-600 text-white uppercase tracking-wider cursor-pointer hover:bg-blue-700 transition-all select-none group"
